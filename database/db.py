...
class Db:
    ...
    async def insert_awb(self, awb, pieces, weight, volume, cargo, departure, destination, flight, date, booking_status, arrival_status, user_id):
        try:
            async with self.pool.acquire() as conn:
                await conn.execute("""
                ...
                """, awb, pieces, weight, volume, cargo.upper(), departure.upper(), destination.upper(), flight.upper(), date, booking_status.upper(), arrival_status.upper(), str(user_id))
        except Exception as e:
            print(f'Error: {e}')
